...
//Cold, blue particles
//Extends GameObject
public class T2 extends GameObject{
	//T2 object constructor
	public T2(int width, int height, Side side) {
		...
		this.velocityX = r.nextInt(3) + 1;
		this.velocityY = r.nextInt(3) + 1;
		velX = velocityX;
		velY = velocityY;
	}
	...
	public void tick() {
		...
		if(mouseClicked == false && side == Side.Right) {
		...
		//if it reaches the wall, then the particle can move to the left box
			...
			if(x - velX - velX< width * FOUR_EIGHTH && System.currentTimeMillis() - timer > 1250) {
		...
		else if(mouseClicked == false && side == Side.Left) {
		...
		//if it reaches the wall, then the particle can move to the right box
			...
			if(x + velX + velX > width * FOUR_EIGHTH - diameter && 
					System.currentTimeMillis() - timer > 1250) {
				if(y + velY > height * TWO_EIGHTH && y + velY < height * FOUR_EIGHTH) {
					side = Side.Right;
					velX = velocityX;
					timer = System.currentTimeMillis();
